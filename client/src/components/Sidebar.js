export default function Sidebar({ tab, setTab }) {
  return (
    <div
      className="d-flex flex-column flex-shrink-0 bg-white border-end h-100"
      style={{ width: "260px", transition: "width 0.3s" }}
    >
      {/* --- Internal CSS for Interactions --- */}
      <style>
        {`
          .nav-link {
            transition: all 0.2s ease-in-out;
            border-radius: 12px; /* Modern rounded corners */
            font-weight: 500;
            color: #6c757d; /* Bootstrap text-muted */
            margin-bottom: 4px;
          }
          
          /* Hover State */
          .nav-link:hover {
            background-color: #f8f9fa; /* bg-light */
            color: #212529; /* text-dark */
            transform: translateX(4px);
          }

          /* Active State: Soft Blue Pill */
          .nav-link.active {
            background-color: rgba(13, 110, 253, 0.1); /* Primary with opacity */
            color: #f8f9fa !important; /* text-primary */
            font-weight: 600;
          }
          
          .nav-icon {
            font-size: 1.3rem;
            min-width: 24px;
          }
        `}
      </style>

      {/* --- Brand / Logo Section --- */}
      <div className="p-4 d-flex align-items-center gap-3">
        <div
          className="d-flex align-items-center justify-content-center text-white rounded-3 shadow-sm"
          style={{ width: "36px", height: "36px" }}
        >
          <img
            src="/logo-hehe.png"
            alt="Logo"
            style={{ width: "24px", height: "24px" }}
          />
        </div>
        <div>
          <h6
            className="m-0 fw-bold text-dark tracking-tight"
            style={{ lineHeight: "1" }}
          >
            FindBack
          </h6>
          <small className="text-muted" style={{ fontSize: "0.7rem" }}>
            Admin Console
          </small>
        </div>
      </div>

      <div className="px-3 py-2">
        <small
          className="text-uppercase text-muted fw-bold"
          style={{ fontSize: "0.65rem", letterSpacing: "1px" }}
        >
          Main Menu
        </small>
      </div>

      {/* --- Navigation Links --- */}
      <ul className="nav nav-pills flex-column px-3 mb-auto">
        <li className="nav-item">
          <button
            className={`nav-link w-100 text-start d-flex align-items-center px-3 py-2 ${
              tab === "dashboard" ? "active" : ""
            }`}
            onClick={() => setTab("dashboard")}
          >
            <i
              className={`bx ${
                tab === "dashboard" ? "bxs-dashboard" : "bx-grid-alt"
              } nav-icon me-3`}
            ></i>
            Dashboard
          </button>
        </li>

        <li className="nav-item">
          <button
            className={`nav-link w-100 text-start d-flex align-items-center px-3 py-2 ${
              tab === "users" ? "active" : ""
            }`}
            onClick={() => setTab("users")}
          >
            <i
              className={`bx ${
                tab === "users" ? "bxs-user-account" : "bx-user"
              } nav-icon me-3`}
            ></i>
            Manage Users
          </button>
        </li>

        <li className="nav-item">
          <button
            className={`nav-link w-100 text-start d-flex align-items-center px-3 py-2 ${
              tab === "items" ? "active" : ""
            }`}
            onClick={() => setTab("items")}
          >
            <i
              className={`bx ${
                tab === "items" ? "bxs-package" : "bx-box"
              } nav-icon me-3`}
            ></i>
            Items Registry
          </button>
        </li>
      </ul>
    </div>
  );
}
